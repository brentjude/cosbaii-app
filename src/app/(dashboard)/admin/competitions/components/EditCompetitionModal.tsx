...
interface EditCompetitionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: EditCompetitionData) => Promise<void>;
...
const EditCompetitionModal = ({
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof EditCompetitionData, string>>>({});
  ...
  useEffect(() => {
    if (isOpen && competition) {
      setFormData({
        ...
        eventDate: competition.eventDate.split("T")[0], // Format date for input
  ...
  }, [isOpen, competition, onClearError]);
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof EditCompetitionData, string>> = {};
  ...
  const handleSave = () => {
    if (!loading && validateForm()) {
      const cleanedData: EditCompetitionData = {
        ...formData,
    ...
    <dialog className="modal modal-open">
      <div className="modal-box max-w-5xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h3 className="font-bold text-lg">Edit Competition</h3>
          <button
        ...
        {errorMessage && (
          <div className="alert alert-error mb-4">
            ...
            <div>
              <p className="font-medium">Failed to update competition</p>
